...
exports.googleLogin = async (req, res) => {
  try {
    ...
    const ticket = await client.verifyIdToken({
      idToken: token,
      audience:
        "188661492944-ht5l2hftpi9et0kjettv9hkhgvg4l98o.apps.googleusercontent.com",
    });
    ...
    const jwtToken = jwt.sign(jwtTokenPayload, process.env.SECRET, {
      expiresIn: "2d",
    });
    ...
    return res
      ...
      .status(200)
      .setHeader("Authoriztion", `Bearer ${token}`)
      .json({
        success:true,
        message:"Logged in successfully",
        token:jwtToken,
    ...
    return res.status(500).json({
        success:false,
        message:"Login through google Failed"
    })
  }
};
